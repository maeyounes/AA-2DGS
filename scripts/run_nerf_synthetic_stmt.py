...
output_dir = "aa_output/benchmark_nerf_synthetic_stmt"
...
def train_scene(gpu, scene, factor):
    cmd = f"OMP_NUM_THREADS=4 CUDA_VISIBLE_DEVICES={gpu} python train.py -s {dataset_dir}/{scene} -m {output_dir}/{scene} --eval --white_background --lambda_normal 0.0 --port {6209+int(gpu)}  --kernel_size 0.1"
    print(cmd)
    if not dry_run:
        os.system(cmd)
